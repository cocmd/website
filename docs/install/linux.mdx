...
To use Cocmd on Linux, you first need to determine whether your system is running a X11 or Wayland desktop environment.
...
Cocmd ships with a `.deb` package, making the installation convenient on Debian-based systems.
...
in the file `cocmd-debian-x11-amd64-sha256.txt`
...
```
sudo apt install ./cocmd-debian-x11-amd64.deb
```
...
wget -O ~/opt/Cocmd.AppImage '${getStableDownloadLink("LINUX_X11_APP_IMAGE_DOWNLOAD_URL")}'
# Make it executable
...
To install the Cocmd's AppImage, open a terminal and follow these steps:
...
If you are using Ubuntu, you can also install Cocmd by simply running this command:

```bash
sudo snap install cocmd --classic --channel=latest/edge
```
...
* Cocmd heavily relies on [cargo make](https://github.com/sagiegurari/cargo-make) for the various packaging
...
At this point, you should have the `cocmd` binary available in the `target/release/` directory.

#### Installing Cocmd

Once you've compiled Cocmd, you can move it into the final location. A good option would be the `/usr/local/bin` folder:

```
sudo mv target/release/cocmd /usr/local/bin/cocmd
```
...
> * If you are using Gnome, Cocmd causes a small "flicker" when using the clipboard backend. This does not happen
> on Sway.
> * If you connect a new keyboard, you will need to manually restart Cocmd with `cocmd restart`, otherwise
> Cocmd won't detect it.
>
> If you encounter a bug, please [open an issue on GitHub](https://github.com/cocmd/cocmd/issues),
...
If you want to verify the correctness of the archive, in the Github Releases page you will find the SHA256 hash 
in the file `cocmd-debian-wayland-amd64-sha256.txt`
...
```
sudo apt install ./cocmd-debian-wayland-amd64.deb
```
...
Once you've compiled Cocmd, you can move it into the final location. 
